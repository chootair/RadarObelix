...
class ObelixClient1090Thread : public QThread
{
    Q_OBJECT
public:
  explicit ObelixClient1090Thread(QHostAddress pHostAddr, uint pPortNo, QObject *parent = 0);
  virtual ~ObelixClient1090Thread();

  void SetTrackTableParameter(QHash<uint16_t, T_ObelixTrack>* pAircaftTablePtr,
...
private:
  ...
  void FromSbs1ToObelix(T_1090MsgSbs1 &pSbs1, T_ObelixTrack &pObelix);
...
private:
  ...
  uint         mPortNo;
  //
  QHash<uint16_t, T_ObelixTrack>* mAircaftTablePtr;
